...
function ProjectCard({ project, liveLabel, noPreview }) {
  ...
  return (
    ...
    >
      <div className="h-48 bg-[#0f0505]/60 border-b border-[#3d1515]/50 overflow-hidden relative">
        {project.preview ? (
          <img
            src={project.preview}
        ...
        <div className={`fallback absolute inset-0 flex-col items-center justify-center gap-2 ${project.preview ? 'hidden' : 'flex'}`}>
          ...
          <span className="text-xs text-[#a87070]">{noPreview}</span>
        </div>
      </div>
      ...
      <div className="p-6">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {project.tags.map((tag, ti) => {
            ...
            return (
              <span key={tag} className={`px-2 py-0.5 bg-[#0f0505]/60 border ${colors[ti % colors.length]} text-xs font-body rounded`}>
                {tag}
              </span>
            )
          })}
    ...
    <section id="portfolio" className="py-24 px-6">
      <div className="max-w-6xl mx-auto">
        <div data-aos="fade-up" className="mb-16">
          <p className="text-[#a87070] text-sm tracking-widest uppercase font-body mb-2">{t.portfolio.subtitle}</p>
          <h2 className="font-heading font-black text-4xl md:text-5xl bg-gradient-to-r from-red-400 to-rose-500 bg-clip-text text-transparent">{t.portfolio.title}</h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project, i) => (
            <div key={i} data-aos="fade-up" data-aos-delay={i * 100}>
              <ProjectCard
                project={project}
                liveLabel={t.portfolio.liveDemo}
                noPreview={t.portfolio.noPreview}
              />
            </div>
          ))}
        </div>
